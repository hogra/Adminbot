            ...
            if event.from_chat:
                ...
                if db_sess.query(group.Group).filter(group.Group.groupid == str(gid)).first() is not None:
                    ...
                    try:
                        ...
                        pass
                if db_sess.query(group.Group).filter(group.Group.groupid == str(gid)).first() is not None and \
                        len(msg) > 0:
                    # Если сообщение является текстом:
                    cru = CurUser(event.object.message['from_id'], gid)
                    # Сохраняю параметры отправителя (того, кто пишет)
                    ...
                    if cru.stat[:3] == 'мут': # Если отправитель в муте
                        if datetime.strptime(cru.stat[4:], '%d.%m.%Y %H:%M:%S') <= datetime.now():
                            # Если время мута истекло
                            streg(gid, cru.uid, 'пользователь')
                            # Отправителю снова присваивается статус "пользователь"
                    ...
                    if msg[0] == '!' and cru.stat == 'админ':
                        ...
                        if msg[1:6] == 'админ': # Команда "админ"
                            if ans is not False: # Если есть получатель
                                ...
                                sender(gid,
                                       'Кого назначать-то? Напишите вашу команду в ответ на сообщение этого '
                            ...
                            if ans is not False:
                                if ans.stat == 'админ': # Если получатель - админ
                                    if cru.uid in getadmins(gid): # И отправитель - организатор
                                        ...
                                        sender(gid, 'Разбирайтесь со своими проблемами сами, или позовите организатора')
                                else:
                                    sender(gid, f"Куда ему падать еще ниже?")
                            else:
                                sender(gid, 'Кого снимать-то? Напишите вашу команду в ответ на сообщение '
                                            'провинившегося')
                            ...
                            if msg[6:11] == 'слово': # Добавление запрещенного слова
                                ...
                                sender(gid, 'Вы не ввели ключ, о ключах прочитайте '
                            ...
                            if ans is not False:
                                ...
                                sender(gid, 'Кого прощать-то? Напишите вашу команду в ответ на сообщение этого '
                                            'бедолагу')
                            ...
                            if ans is not False:
                                    ...
                                    if cru.uid in getadmins(gid):
                                        try:
                                            ...
                                            if ans.pred + 1 >= cgr.pred:
                                                ...
                                                print(f'    Пользователь {cru.name, cru.sname} был кикнут из-за '
                                                      f'достижения максимального количества предупреждений')
                                                db_sess.query(users.User).filter(users.User.userid == ans.uid,
                                                                                 users.User.groupid == gid).delete()
                                                db_sess.commit()
                                                vk.messages.removeChatUser(chat_id=gid, user_id=ans.uid)
                                ...
                                sender(gid,
                                       'Кого предупреждать-то? Напишите вашу команду в ответ на сообщение этого '
                                       'нарушителя')
                            ...
                            if ans is not False:
                                ...
                                sender(gid, 'Кого размутить-то? Напишите вашу команду в ответ на сообщение этого '
                                            'молчуна')
                            ...
                            if msg[1:5] != 'стат' and msg[1:4] != 'топ' and msg[1:5] != 'даты':
                                sender(gid, 'А где комманда? вот тебе список, не '
                                            'волнуйся\nvk.com/@-219582896-komandy-bota-tovarisch-admin')
                    ...
                    if msg[:5] == '!стат':
                        ...
                        top = db_sess.query(users.User.name, users.User.surname,
                                            users.User.symbols).filter(users.User.groupid == gid).all()
                        ...
                        top = db_sess.query(users.User.name, users.User.surname,
                                            users.User.date).filter(users.User.groupid == gid).all()
                    ...
                    sender(gid,
                           'Товарищ, похоже эта беседа не зарегистрирована!\nЧтобы зарегистрировать беседу '
                           'предоставьте боту права администратора, доступ ко всей переписке и напишите команду "!За '
                           'работу"')
    print(input())